...
pub fn cubic(x: FLOAT, p: &[FLOAT; 4]) -> FLOAT {
    p[0] + x * (p[1] + x * (p[2] + x * p[3]))
}

pub fn cubic_diff(x: FLOAT, p: &[FLOAT; 4]) -> FLOAT {
    p[1] + x * (2.0 * p[2] + x * 3.0 * p[3])
}

pub fn cubic_diff2(x: FLOAT, p: &[FLOAT; 4]) -> FLOAT {
    2.0 * p[2] + x * 6.0 * p[3]
}

pub fn cubic_diff3(_x: FLOAT, p: &[FLOAT; 4]) -> FLOAT {
    6.0 * p[3]
}

pub fn cubic_log(x: FLOAT, p: &[FLOAT; 4]) -> FLOAT {
    cubic(x, p).exp()
}
...
pub fn cubic_spline_0<const N: usize, const M: usize>(
    ...
    let mut tmp = vec![[0.0 as FLOAT; 3]; N + 1];
    tmp[0][0] = 0.0;
    tmp[0][2] = 2.0 * a;
    for i in 0..N {
        tmp[i + 1][0] = arr[i][id];
    ...
    let mut pols: Box<[[FLOAT; 4]; N]> = boxarray(0.0);
    pols[0][0] = tmp[0][0];
    pols[0][1] = a * tmp[0][1];
...
pub fn cubic_spline<const N: usize, const M: usize>(
    ...
    let mut tmp: Box<[[FLOAT; 4]; N]> = boxarray(0.0);
    for i in 0..N {
        tmp[i][0] = arr[i][id];
        tmp[i][2] = 4.0;
    }
